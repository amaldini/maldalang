...
public class CSharpTranspiler
{
    ...
    private void WriteNeuralLayerDispatch(string typeName, string localName)
    {
        WriteIndent();
        _output.Append("else if (instance is ");
        ...
        _output.Append(localName);
        _output.AppendLine(".CallMethod(methodName, runtimeArgs, MaldaLang.Runtime.TranspiledBuiltinRuntime.GetOrCreateInterpreter());");
        _indentLevel--;
        WriteIndent();
        _output.AppendLine("}");
    }
    ...
    private void GenerateRuntimeHelpers()
    {
        ...
        WriteIndent();
        _output.AppendLine("private static readonly System.Runtime.CompilerServices.ConditionalWeakTable<System.Collections.Generic.List<object>, System.Collections.Generic.List<MaldaLang.Interpreter.RuntimeValue>> __objectListToRvListCache = new System.Runtime.CompilerServices.ConditionalWeakTable<System.Collections.Generic.List<object>, System.Collections.Generic.List<MaldaLang.Interpreter.RuntimeValue>>();");
        _output.AppendLine();
        WriteIndent();
        ...
        WriteIndent();
        _output.AppendLine("__objectListToRvListCache.Add(result, runtimeValueList);");
        WriteIndent();
        ...
        _indentLevel++;
        WriteIndent();
        _output.AppendLine("if (__objectListToRvListCache.TryGetValue(list, out var bridgedSource))");
        WriteIndent();
        _output.AppendLine("    RefreshBridge(list, bridgedSource);");
        WriteIndent();
        ...
        _indentLevel++;
        WriteIndent();
        _output.AppendLine("if (__rvListToObjectListCache.TryGetValue(runtimeValueList, out var existing))");
        WriteIndent();
        _output.AppendLine("{");
        _indentLevel++;
        WriteIndent();
        _output.AppendLine("RefreshBridge(existing, runtimeValueList);");
        WriteIndent();
        _output.AppendLine("return existing;");
        _indentLevel--;
        WriteIndent();
        _output.AppendLine("}");
        WriteIndent();
        ...
        _output.AppendLine();
        WriteIndent();
        _output.AppendLine("private static void RefreshBridge(List<object> arr, System.Collections.Generic.List<MaldaLang.Interpreter.RuntimeValue> source)");
        WriteIndent();
        _output.AppendLine("{");
        _indentLevel++;
        WriteIndent();
        _output.AppendLine("var count = arr.Count < source.Count ? arr.Count : source.Count;");
        WriteIndent();
        _output.AppendLine("for (var i = 0; i < count; i++)");
        WriteIndent();
        _output.AppendLine("{");
        _indentLevel++;
        WriteIndent();
        _output.AppendLine("var item = source[i];");
        WriteIndent();
        _output.AppendLine("if (item.Type == MaldaLang.Interpreter.ValueType.Integer) arr[i] = item.AsInteger();");
        WriteIndent();
        _output.AppendLine("else if (item.Type == MaldaLang.Interpreter.ValueType.Float) arr[i] = item.AsFloat();");
        WriteIndent();
        _output.AppendLine("else if (item.Type == MaldaLang.Interpreter.ValueType.String) arr[i] = item.AsString();");
        WriteIndent();
        _output.AppendLine("else if (item.Type == MaldaLang.Interpreter.ValueType.Boolean) arr[i] = item.AsBoolean();");
        _indentLevel--;
        WriteIndent();
        _output.AppendLine("}");
        _indentLevel--;
        ...
        _output.AppendLine();
        WriteIndent();
        _output.AppendLine("public static void SyncIndexed(List<object> arr, int index, object? newValue)");
        WriteIndent();
        _output.AppendLine("{");
        _indentLevel++;
        WriteIndent();
        _output.AppendLine("if (__objectListToRvListCache.TryGetValue(arr, out var source) && (uint)index < (uint)source.Count)");
        WriteIndent();
        _output.AppendLine("    source[index] = ToRuntimeValue(newValue);");
        _indentLevel--;
        WriteIndent();
        _output.AppendLine("}");
        _output.AppendLine();
        ...
        _output.AppendLine("}");
        WriteIndent();
        _output.AppendLine("else if (instance is MaldaLang.BuiltIns.DeclaredContextInstance declaredContext)");
        WriteIndent();
        _output.AppendLine("{");
        _indentLevel++;
        WriteIndent();
        _output.AppendLine("result = declaredContext.CallMethod(methodName, runtimeArgs, MaldaLang.Runtime.TranspiledBuiltinRuntime.GetOrCreateInterpreter());");
        _indentLevel--;
        WriteIndent();
        _output.AppendLine("}");
        WriteIndent();
        ...
        WriteIndent();
        _output.AppendLine("SyncIndexed(arr, i, newValue);");
        WriteIndent();
        _output.AppendLine("return;");
        _indentLevel--;
        WriteIndent();
    ...
    private void TranspilePrompt(PromptDeclaration promptDecl, List<ClassDeclaration> classes, List<SchemaDeclaration> schemas)
    {
        ...
        WriteIndent();
        _output.AppendLine("var attachmentsValue = RuntimeHelpers.GetPromptObjectField(bodyValue, \"attachments\");");
        WriteIndent();
        _output.AppendLine("var attachments = MaldaLang.BuiltIns.PromptAttachment.ParseListOrNull(attachmentsValue);");
        ...
        else
        {
            ...
            WriteIndent();
            _output.AppendLine("System.Collections.Generic.List<MaldaLang.BuiltIns.PromptAttachment>? attachments = null;");
            _output.AppendLine();
                    ...
                    if (stmt is PromptBodyStatement bodyStmt)
                    {
                        ...
                        switch (keyword)
                        {
                            ...
                            case "examples":
                                ...
                                break;
                            case "attachments":
                                _output.AppendLine("attachments = MaldaLang.BuiltIns.PromptAttachment.ParseListOrNull(" + keyword + "Value);");
                                break;
                        }
        ...
        WriteIndent();
        _output.AppendLine("if (attachments != null && attachments.Count > 0)");
        WriteIndent();
        _output.AppendLine("{");
        _indentLevel++;
        WriteIndent();
        _output.Append("MaldaLang.BuiltIns.PromptAttachment.ApplyParameterInterpolation(attachments, new System.Collections.Generic.List<string> { ");
        for (int i = 0; i < promptDecl.Parameters.Count; i++)
        {
            if (i > 0) _output.Append(", ");
            _output.Append("\"");
        ...
        WriteIndent();
            _output.AppendLine("__promptInstance = new MaldaLang.BuiltIns.PromptInstance(__extractSystem, __extractUser, __promptInstance.Model, __promptInstance.Temperature, null, __promptInstance.MaxTokens, __extractFormat, __promptInstance.Examples, __promptInstance.WithinTimeoutMs, null, __promptInstance.Budget, __promptInstance.ReturnType, __promptInstance.Attachments);");
        WriteIndent();
        ...
        WriteIndent();
            _output.AppendLine("__promptInstance = new MaldaLang.BuiltIns.PromptInstance(__promptInstance.System, __baseUser + \"\\n\\n\" + __repair, __promptInstance.Model, __promptInstance.Temperature, __promptInstance.Tools, __promptInstance.MaxTokens, __promptInstance.ResponseFormatSchema, __promptInstance.Examples, __promptInstance.WithinTimeoutMs, __promptInstance.Gather, __promptInstance.Budget, __promptInstance.ReturnType, __promptInstance.Attachments);");
        _indentLevel--;
        WriteIndent();
        _output.AppendLine("}");
