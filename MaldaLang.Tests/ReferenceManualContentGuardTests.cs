...
public class ReferenceManualContentGuardTests
{
    ...
    public void ReservedWordLists_CoverEveryLexerKeyword()
    {
        ...
        foreach (var page in new[] { "03-lexical-structure.html", "37-appendix.html" })
        {
    ...
    public void WebUiChapter_NamesEveryRegisteredControlType()
    {
        ...
        var chapter = File.ReadAllText(Path.Combine(ManualDir, "25-web-ui.html"));
        var missing = types.Where(type => !chapter.Contains($"ui.{type}", StringComparison.Ordinal)).ToList();
        Assert.True(
            missing.Count == 0,
            "25-web-ui.html must name every UiControlSpecRegistry type as ui.<type>: " +
            string.Join(", ", missing));
    }
    ...
    private static HashSet<string> ReservedWordsListedIn(string page)
    {
        ...
        var marker = page == "37-appendix.html" ? "Appendix A: Reserved Words" : "Complete Keyword List";
