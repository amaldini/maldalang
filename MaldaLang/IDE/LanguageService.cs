...
public class LanguageService : ILanguageService
{
    ...
    private string? GetBuiltInFunctionInfo(string name)
    {
        return name switch
        {
            ...
            "executePlan" => "function executePlan(plan: object, agent: Agent | AgentTeam) -> object\nValidates the plan, topo-sorts steps by dependsOn, then runs think() with the step description plus prior step outputs. plan.think: false skips think() (offline). A team requires each step.role (or step.agent) to name a member, and a dependsOn hop between different roles must match a declared relation. A reject hop runs only when the predecessor is not approved; other hops run only when it is approved (default true; set step.approved or JSON output). Results include prompt, rel, approved, and skipped. Returns { planId, completed, failed, skipped, results }.",
            ...
            "run" => "function AgentTeam.run(plan) -> object\nexecutePlan(plan, team). Team plans need step.role and legal hops. plan.think: false skips think(); reject hops follow the predecessor approved verdict.",
