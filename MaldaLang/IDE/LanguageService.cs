...
public class LanguageService : ILanguageService
{
    ...
    private string? GetBuiltInFunctionInfo(string name)
    {
        return name switch
        {
            ...
            "executePlan" => "function executePlan(plan: object, agent: Agent | AgentTeam) -> object\nValidates the plan, topo-sorts steps by dependsOn, then runs think() with the step description plus prior step outputs. A team requires each step.role (or step.agent) to name a member, and a dependsOn hop between different roles must match a declared relation. Results include prompt and, for a role hop, rel. Returns { planId, completed, failed, results }.",
            ...
            "run" => "function AgentTeam.run(plan) -> object\nexecutePlan(plan, team). Team plans need step.role and legal hops.",
