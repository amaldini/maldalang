...
public class LanguageService : ILanguageService
{
    ...
    private string? GetBuiltInFunctionInfo(string name)
    {
        return name switch
        {
            ...
            "executePlan" => "function executePlan(plan: object, agent: Agent | AgentTeam) -> object\nValidates the plan, topo-sorts steps by dependsOn, then runs think() with the step description plus prior step outputs. A team requires each step.role (or step.agent) to name a member, and a dependsOn hop between different roles must match a declared relation. Results include prompt. Returns { planId, completed, failed, results }.",
            ...
            "handoff" => "function AgentTeam.handoff(from, to, payload, think?) -> object\nAllows a hop only along a declared edge. If the edge has contract, runs validate(contract, payload). think: true (or { think: true }) then calls to.think(payload) and returns { ok, data, response }. Default is validate-only. Returns { ok, data } or { ok: false, error }.",
