...
public class LanguageService : ILanguageService
{
    ...
    private string? GetBuiltInFunctionInfo(string name)
    {
        return name switch
        {
            ...
            "executePlan" => "function executePlan(plan: object, agent: Agent | AgentTeam) -> object\nValidates the plan, topo-sorts steps by dependsOn, then runs think(step.description). A team requires each step.role (or step.agent) to name a member, and a dependsOn hop between different roles must match a declared relation. Returns { planId, completed, failed, results }.",
