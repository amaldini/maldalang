...
/// <summary>
/// SQLite database client for MALDA with connection management, raw SQL queries, parameterized queries, and query builder support.
/// </summary>
public class SqliteClientInstance : ObjectInstance, IDisposable
{
    ...
    public SqliteClientInstance() : base(null)
    {
    }

    public SqliteClientInstance(string? connectionString) : base(null)
    {
    ...
    {
        if (_disposed)
            throw new ObjectDisposedException(nameof(SqliteClientInstance), "SqliteClient has been disposed.");
    }
    ...
    public override RuntimeValue Get(string name, ClassDefinition? accessingClass = null)
    {
        ...
        throw new Exception($"Undefined property '{name}' on SqliteClient.");
    }
    ...
    private void AddParameters(SqliteCommand command, RuntimeValue? parameters)
    {
        ...
        if (paramsObj is JsonObject jsonObj)
        {
            ...
            foreach (var kvp in props)
            {
                ...
                var param = new SqliteParameter(paramName, ConvertToDbValue(kvp.Value));
                command.Parameters.Add(param);
            }
            return;
        }

        // Handle DictionaryInstance (used by transpiled object literals)
        if (paramsObj is DictionaryInstance dictInstance)
        {
            var entries = dictInstance.GetEntries();
