...
/// <summary>
/// PostgreSQL database client for MALDA with connection management, raw SQL queries, parameterized queries, and query builder support.
/// </summary>
public class PostgresClientInstance : ObjectInstance, IDisposable
{
    ...
    public PostgresClientInstance() : base(null)
    {
    }
    
    public PostgresClientInstance(string? connectionString) : base(null)
    {
    ...
    {
        if (_disposed)
            throw new ObjectDisposedException(nameof(PostgresClientInstance), "PostgresClient has been disposed.");
    }
    ...
    public override RuntimeValue Get(string name, ClassDefinition? accessingClass = null)
    {
        ...
        throw new Exception($"Undefined property '{name}' on PostgresClient.");
    }
    ...
    private RuntimeValue Query()
    {
        ...
        _parameterCounter = 0;
        var sql = _queryBuilder.BuildSqlPostgres(ref _parameterCounter);
        ...
        {
            allParams.Set($"${paramIndex++}", condition.Value);
        }
    ...
    private RuntimeValue Insert(string table, RuntimeValue values)
    {
        ...
        _parameterCounter = 0;
        var paramPlaceholders = string.Join(", ", properties.Keys.Select((k, i) => $"${++_parameterCounter}"));
        var sql = $"INSERT INTO {table} ({columns}) VALUES ({paramPlaceholders})";
        
        var paramsObj = new JsonObject();
        var paramIndex = 1;
        foreach (var kvp in properties)
        {
            paramsObj.Set($"${paramIndex++}", kvp.Value);
        }
    ...
    private RuntimeValue Update(string table, RuntimeValue values)
    {
        ...
        _parameterCounter = 0;
        var setClause = string.Join(", ", properties.Keys.Select((k, i) => $"{k} = ${++_parameterCounter}"));
        var sql = $"UPDATE {table} SET {setClause}";
        
        // Add WHERE clause
        var whereSql = _queryBuilder.BuildWhereClausePostgres(ref _parameterCounter);
        sql += " WHERE " + whereSql;
        ...
        foreach (var kvp in properties)
        {
            allParams.Set($"${currentParam++}", kvp.Value);
        }
        foreach (var condition in _queryBuilder.WhereConditions)
        {
            allParams.Set($"${currentParam++}", condition.Value);
        }
    ...
    private void AddParameters(NpgsqlCommand command, RuntimeValue? parameters)
    {
        ...
        if (paramsObj is JsonObject jsonObj)
        {
            ...
            // Sort by parameter number for PostgreSQL ($1, $2, etc.)
            var sortedParams = props.OrderBy(kvp => 
            {
                var key = kvp.Key;
                if (key.StartsWith("$"))
                {
                    if (int.TryParse(key.Substring(1), out var num))
                        return num;
                }
                return int.MaxValue;
            }).ToList();
            ...
            foreach (var kvp in sortedParams)
            {
                ...
                {
                    // Convert named parameter to positional
                    paramName = $"${++_parameterCounter}";
                }
                var param = new NpgsqlParameter(paramName, ConvertToDbValue(kvp.Value));
        ...
        foreach (var kvp in properties)
        {
            var param = new NpgsqlParameter($"${paramIndex++}", ConvertToDbValue(kvp.Value));
            command.Parameters.Add(param);
        }
    }
    ...
    private class QueryBuilder
    {
        ...
        public string BuildSql()
        {
            if (Type == QueryType.Select)
            {
                ...
                {
                    int dummyCounter = 0;
                    sql += " WHERE " + BuildWhereClausePostgres(ref dummyCounter);
                }
                return sql;
            }
        ...
        public string BuildSqlPostgres(ref int paramCounter)
        {
            if (Type == QueryType.Select)
            {
                var columns = Columns.Count > 0 ? string.Join(", ", Columns) : "*";
                ...
                if (WhereConditions.Count > 0)
                {
                    sql += " WHERE " + BuildWhereClausePostgres(ref paramCounter);
                }
                return sql;
            }
            throw new RuntimeException("Unsupported query type");
        ...
        public string BuildWhereClausePostgres(ref int paramCounter)
        {
            var conditions = new List<string>();
            foreach (var condition in WhereConditions)
            {
                conditions.Add($"{condition.Column} {condition.Operator} ${++paramCounter}");
            }
