...
- **"Create module file" quick fix:** the `malda-import` warning now offers a `textDocument/codeAction` quick fix that creates the resolved target through a `CreateFile` resource operation (`documentChanges`), so the `suggestedFix` is actionable in LSP clients instead of advice only.
- **One source-key rule across LSP handlers:** `textDocument/documentSymbol`, `references`, `prepareRename`, `rename`, and `documentHighlight` now pass the document's real filesystem path (via `WorkspaceDocumentManager.GetSourceKey`) like `definition` already did, so an `include` / file `import` that resolves never renders as `(not found)` and caret-on-module requests base on the document's directory on Windows.
