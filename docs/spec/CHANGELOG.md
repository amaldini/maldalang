...
- **"Create module file" quick fix:** the `malda-import` warning now offers a `textDocument/codeAction` quick fix that creates the resolved target through a `CreateFile` resource operation (`documentChanges`), so the `suggestedFix` is actionable in LSP clients instead of advice only. The action is offered only when the client declares `workspace.workspaceEdit.resourceOperations` including `create`.
- **One source-key rule across LSP handlers:** `textDocument/documentSymbol`, `references`, `prepareRename`, `rename`, and `documentHighlight` now pass the document's real filesystem path (via `WorkspaceDocumentManager.GetSourceKey`) like `definition` already did, so an `include` / file `import` that resolves never renders as `(not found)` and caret-on-module requests base on the document's directory on Windows. `didOpen` / `didChange` diagnostics use `WorkspaceDocumentManager.GetLocalFilePath`, which shares that resolution and stays null for an unsaved buffer.
...
- **`agents.define` / `agents.team`:** namespaced helpers (no flat alias, no new keyword) that build an agent from `{ name, role, instructions, kind?, tools?, memoryScope?, workingDirectory?, includeSymbols?, readOnly?, prdAuthorOnly? }` and bind an array of those specs to a directed graph. Extra graph-literal edge keys besides `from` / `to` / `weight` / `properties` (`rel`, `contract`, …) become edge properties. Allowed `rel`: `handoff`, `delegate`, `review`, `consult`, `reject`. Allowed `kind`: `Agent` (default), `CodingAgent`, `GitAgent`, `HumanAgent`, `DevAgent`, `MALDACodingAgent`. Specialized kinds use the same tool bundles as `new CodingAgent` / `new DevAgent` / … Construction does not auto-download a local model (the three-client constructors run with a null client). `workingDirectory` is required-optional on specialized kinds only; `includeSymbols` / `readOnly` / `prdAuthorOnly` apply only to `DevAgent`. Unknown nodes, rels, or kinds fail before any LLM call. `delegate` / `consult` edges install `addSubAgent`. `team.handoff(from, to, payload)` allows only a declared edge and `validate`s `contract` when set. `executePlan(plan, team)` (or `team.run(plan)`) requires each step to name `role` (or `agent`). `agent.kind` is the class tag. Interpreter and C# transpile agree. JavaScript: n/a (host-only). Examples: `Examples/Agents/agent_team_graph.malda`, `Examples/Agents/agent_team_kinds.malda`. Few-shots: `docs/llm/few-shot/34_agents_team.malda`, `docs/llm/few-shot/35_agents_kind.malda`.
...
| 2026-09-23 | MINOR: `nn.*` groups activations, local derivatives, `dense` / `denseBackward`, `mseGrad`, `softmaxGrad`. `relu`, `mse`, and `crossEntropyFromLogits` are only on `nn`. `math.sigmoid` / `tanh` / `softmax` stay |
