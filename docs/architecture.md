...
Server-driven `ui.*` trees, patch protocol, and `MaldaLang.UIHost` wiring are documented in [`docs/ui-framework.md`](ui-framework.md). Language API for components and controls: [`ReferenceManual/26-web-ui.html`](../ReferenceManual/26-web-ui.html).
...
| `docs/spec/ship-contract.md` | Host ship oracles: pair / workflow journal / jobs journal / HTTP status+body / Tier 0 C# slice / n/a |
...
| [`docs/roadmap-games.md`](roadmap-games.md) | Browser games kit (`game.*` / `three.*` JS-only; G0–G17 landed) |
| [`docs/roadmap-neural.md`](roadmap-neural.md) | Neural-nets kit (`math.*` LA, `nn.*` dense/activations, AI_Theory, host `OnnxModel`; N0–N4 and N6–N10 landed, N5 partial) |
...
- Capability tokens: [`cap.fileRead`](../MaldaLang/BuiltIns/CapStdLib.cs) / `cap.httpGet` / `cap.mcpCall` / `cap.shell` mint unforgeable handles; `cap.read` / `cap.fetch` / `cap.invoke` / `cap.run` (and `io.readFile` / `webFetch` / `callTool` / `runCommand`) consume matching tokens. Object literals cannot forge a token. `@effects("io")` stays a name allow-list. First-contact scaffold: `malda new agent` (`Templates/agent/`).
- Agent teams: [`agents.define`](../MaldaLang/BuiltIns/AgentsStdLib.cs) / `agents.team` bind role specs to a directed graph (`rel` / `contract` on edges). Spec `kind` selects `Agent` / `CodingAgent` / `GitAgent` / `HumanAgent` / `DevAgent` / `MALDACodingAgent` (no auto-download). `executePlan(plan, team)` routes by `step.role`, rejects illegal hops, feeds prior outputs into `think()`, records incoming `rel`, and skips hops that do not match the predecessor `approved` verdict (`reject` only when not approved). `step.out` names a schema / sum type / `program(Api)`; think JSON (or `step.fixture`) is coerced like `await prompt … -> Type` / `evalPrompt`. Failed extract fails the step and skips dependents; unknown `out` fails the plan before `think()`. `plan.think: false` skips `think()` (typed steps then need `fixture`). `team.handoff` / `team.review` / `team.reject` / `team.consult` are typed hops (matching `rel`); validate-only unless `think` is true. `delegate` / `consult` edges also install `addSubAgent`. `decomposeTask(…, team?)` / `team.decompose` seed the roster into the plan prompt. No new keyword.
