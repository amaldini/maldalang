...
- Agent teams: [`agents.define`](../MaldaLang/BuiltIns/AgentsStdLib.cs) / `agents.team` bind role specs to a directed graph (`rel` / `contract` on edges). `executePlan(plan, team)` routes by `step.role`. No new keyword.
