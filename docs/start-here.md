...
The neural-nets kit (`math.matmul`, `nn.dense` / activations, `OnnxModel`) is [Reference Manual 14](../ReferenceManual/14-neural-nets.html). A ten-glyph MLP in that same training step is `Examples/AI_Theory/mnist_digits.malda`. A one-step REINFORCE bandit on that gradient is `Examples/AI_Theory/reinforce_bandit.malda`. After it, the chapter 14 chain is `gradcheck_dense.malda`, `init_scale.malda`, `glyph_holdout.malda`, `momentum_valley.malda`, `grad_clip.malda`, `adam_valley.malda`, `rnn_delay.malda`, `conv_stroke.malda`, `conv_classifier.malda`, `residual_dropout.malda`, `embed_row.malda`, `next_char.malda`, `next_char_mlp.malda`, `layer_norm.malda`, `attention_step.malda`, `positional_encoding.malda`, and `sentence_decoder.malda`. The three-class picture is `malda play Examples/Games/softmax_decision_boundary.malda`. The maintainer plan is [`docs/roadmap-neural.md`](roadmap-neural.md).
