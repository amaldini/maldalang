...
- module-like groups:
  ...
  - `game`: 2D canvas/audio helpers, pixel-buffer blit (`setPixel` / `blitPixels`), images (`loadImage` / `drawImage` / `drawImageRect` / `drawImageEx` / `imageWidth`), a 2D camera (`setCamera`, `setCameraZoom`, `pushCamera` / `popCamera`, `screenToWorld` / `worldToScreen`), size queries (`getCanvasWidth`, `measureText`), pixelated draws (`setPixelated`), `strokeCircle`, key edges (`wasKeyPressed` / `wasKeyReleased`), mouse world coords and edges (`getMouseWorldX`, `wasMousePressed`), touches (`getTouches`), gamepad helpers, overlap queries (`overlapRect` / `overlapCircle`), and swept AABB (`sweepRect`)
