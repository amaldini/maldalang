...
**Status:** N0–N4 landed; N5–N6 docs/examples only  
**Created:** 2026-09-20  
**Audience:** maintainers extending `math.*` and the offline AI_Theory track after Final 1.0
...
[Reference Manual 13](../ReferenceManual/13-built-in-functions.html),
and the out-of-scope list below for engines that stay out of core.

**Bar today:** `math.dot` / `matmul` / `transpose`, activations (`relu` /
`sigmoid` / `tanh`), `math.mse`, the student track through XOR / softmax /
attention / microgpt, a JS decision-boundary playground (`malda play`),
hash-embedding 2D projection + VectorDB neighbors, and host-only
`new OnnxModel(path)` inspect + forward.

**Not in scope:** a `tensor` language type, native autograd, `nn.Linear` /
Adam / DataLoader, GPU training, a general ONNX trainer, new namespaces
(`nn.*` / `tensor.*` / `plot.*`), product apps or vertical packs
(`AGENTS.md`). Serious inference runtimes stay an **optional pack out of
tree** (`MaldaLang.Compiler/OptionalPack/`).
...
1. **Teach the idea, do not hide it.** Named chain-rule gradients stay in
   user code. Builtins replace only the loops every example recopied
   (`matmul`, `sigmoid`).
2. **Extend `math.*`.** Stdlib is soft-frozen: deepen `math` / `str` / `io`;
   do not add `nn.*` or flat aliases (`AGENTS.md`).
...
4. **Three backends for `math.*`.** Interpreter, C# transpile, and JavaScript
   all run the linear-algebra slice. VectorDB / LlamaEmbedder / `OnnxModel`
   stay host-only
