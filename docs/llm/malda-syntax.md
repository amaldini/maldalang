...
- Prompt `-> ReturnType` is **not** static typing. Three supported modes:
  ...
  (`step.approved` or JSON output; default true). `plan.think: false` skips
  `think()` (offline). `decomposeTask(instruction, client?, team?)` (or
  ...
  `Examples/Agents/agent_team_consult.malda`.
  ...
  `docs/llm/few-shot/40_agents_consult.malda`.
