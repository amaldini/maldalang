...
- Prompt `-> ReturnType` is **not** static typing. Three supported modes:
  ...
  reject returns `{ ok, rejected, data }`; consult returns `{ ok, data }` (one-shot
  ask, does not transfer the plan). `executePlan(plan, team)` requires
  ...
  prompt; the step result includes that hop's `rel`. A `reject` hop runs only when
  the predecessor is not approved; other hops run only when it is approved
  (`step.approved` or JSON output; default true). `step.out` names a schema,
  sum type, or `program(Api)`: think JSON (or `step.fixture`) is coerced like
  `await prompt … -> Type` / `evalPrompt`. Failed extract fails the step and
  skips dependents. Unknown `out` fails the plan before `think()`.
  `plan.think: false` skips `think()` (typed steps then need `fixture`).
  `decomposeTask(instruction, client?, team?)` (or
  ...
  `Examples/Agents/agent_team_review.malda`, `Examples/Agents/agent_team_plan_verdict.malda`,
  `Examples/Agents/agent_team_consult.malda`, `Examples/Agents/agent_team_plan_out.malda`.
