...
Full prose catalog: `ReferenceManual/13-built-in-functions.html` and `MaldaLang/BuiltIns/`.
...
`SerialConnection`, `ArduinoConnection`, `OnnxModel`.
...
| `new Conv` / `Embedding` / `Rnn` / `LayerNorm` / `Attention` | Same contract as `Dense`: `forward`, `backward`, `sgd`. No tape |
...
exact arguments rather than guessing.
...
| `prompt name(args) -> SchemaName { … }` + `await` | Typed prompt Mode A: no `gather:` → `response_format` + schema appendix + GBNF on in-process GGUF + validate/repair. Mode B (`tools:`) sends the same format/appendix with tools (no GBNF); if the backend rejects tools+json_schema, retry once without format. Mode C: `gather:` + `-> Type` — tool round then typed extract |
