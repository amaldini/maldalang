...
1. `malda-syntax.md` (always â€” includes array mutation and `$"..."` interpolation)
2. `malda-gotchas.md` (always â€” it is short, and it covers the failures nothing else reports)
3. 2â€“4 files from `few-shot/` matching the task
...
`MaldaLang.Executable.pdb` â€” safe to ignore or delete when shipping only the named exe.
...
**Interpreter â‰  transpile.** `malda prog.malda` and `malda compile --mode transpile` are
...
At end of input, `io.input` returns `""` forever (not `null`) â€” every input loop needs an
...
Do **not** use `"50","25","39" | .\malda.bat â€¦` under Windows PowerShell 5.1 â€” it pipes a
UTF-8 BOM on the first line, so the program sees `ï»¿50` and `toIntOrNull` rejects it. Prefer
...
Put the two together â€” **seed, pipe a transcript, assert on the output** â€” and you can prove
...
a bug to chase â€” check those visuals in an interactive UTF-8 terminal.
