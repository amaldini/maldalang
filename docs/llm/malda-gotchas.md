...
| `var n: int = "abc";` or `f("a")` when `f(x: int)` | Runs at runtime. The language server emits a **Warning** for literal mismatches on variables, fields, call arguments, and `return` vs `-> T`. With `--strict-types` those mismatches are **Errors**. Nothing enforces hints at runtime. | Fix the literal, validate explicitly, or use `toIntOrNull` |
