...
| `var n: int = "abc";`, `n = "a"`, `var p: Person = 1`, `f(s)` when `f(x: int)` and `s: string`, or `var n: int = make()` when `make() -> string` | Runs at runtime. The language server emits a **Warning** for mismatches on literals, `new ClassName()`, identifiers with known hints, and **call results** whose callee declares `-> T` (Tier 0 names, declared/imported class/schema/sum types, host classes; variables, fields, assignment, call arguments, `return` vs `-> T`). With `--strict-types` those mismatches are **Errors**; unknown hint names are also errors. Operators / built-in returns are not inferred. Nothing enforces hints at runtime. | Fix the value, validate explicitly, or use `toIntOrNull` |
| `schema A { b: NotAType; }` | Resolving/validating the schema throws — unknown field types are no longer silently treated as `string`. | Use a JSON primitive (`string`, `int`, …) or another declared schema name (`B` / `B[]`) |
