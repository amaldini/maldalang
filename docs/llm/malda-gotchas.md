...
| `@MCPTool("add", "…")` without a schema (or a JSON blob as the third arg) | Every parameter is advertised as <code>"string"</code>. Numbers and optional fields are lost unless you pass JSON. | Declare `schema AddArgs { a: int; b: int; }` and write `@MCPTool("add", "…", "AddArgs")` (or the identifier `AddArgs`). JSON object strings remain valid for enums/defaults. Example: `Examples/MCP/mcp_schema_tool.malda` |
