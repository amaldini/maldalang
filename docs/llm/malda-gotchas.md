...
| `@effects("io")` plus `io.readFile(path)` / `webFetch(url)` / `runCommand(cmd)` / `callTool(name)` on a tool that takes those as strings | The decorator only **names** the effect. The model can still invent `/etc/passwd`, any URL, any MCP tool, or any argv. The call succeeds. | Host-mint `cap.fileRead` / `cap.httpGet` / `cap.mcpCall` / `cap.shell`, take a relative path/URL/tool/extra argv, `cap.confine` then consume. How-to: `ReferenceManual/12-input-output.html` §12.6. Scaffold: `malda new agent` |
...
| `items.forEach(x => x.done = true)` | Assignment in an **expression** lambda is dropped: the parser keeps only the RHS, so this is `x => true` and `done` is unchanged. No error. Same for `var f = x => x.n = 1`. | `items.forEach(x => { x.done = true; })` — mutations need a block body |
...
| `return x + y` in a method, expecting a top-level `var x` | Bare names in methods are locals, then fields, then the module. A field named `x` wins over the outer `var`. | Rename the field or the global, or pass the outer value as a parameter |
...
| Mixing `@PAGE` HTML strings with `ui.*` trees as if they were the same model | Both run, but they are different UIs: `@PAGE` returns HTML; `ui.mount` / `ui.render` expect node trees from `ui.button(props, …)`, not `"<button>…"`. | Pick one model per surface; see `ReferenceManual/24-web-ui-hub.html` |
...
| `import "typo.malda";` / `include "typo.malda";` | `include` fails at **parse** time; a file `import` fails at **run** time. Neither is silent in the IDE: `malda check --json` reports a `malda-import` Warning naming the resolved path, F12 on the path jumps to the target, and the editor offers a **"Create module file"** quick fix that makes the resolved path. The import still runs the program with the symbol missing. | Fix the path; run `malda check path.malda --json` and look for `malda-import`; or accept the quick fix to create the missing file |
...
| `var api = new RestServer(); api.enableCsrf(...)` | `api` is a **keyword** (`api Name { }`). A statement that starts with `api.` is parsed as an API declaration. | Name the instance `rest` / `server` — `Templates/fullstack/backend/app.malda` |
| `enqueueJob` expecting durable workflow semantics | Jobs are a **lightweight SQLite queue** (`./.malda/jobs.db`, or `MALDA_JOBS_CONNECTION`), not `workflow` / `step` / compensate. | Use `workflow { }` for durable steps; use jobs for fire-and-forget workers |
...
| Two sum types that both declare `Ok` (`type r = Ok\|Err` and `type r2 = Ok\|Warning`) then `var x = Ok("Ciao")` | Constructor tags are **global**. Bare `Ok(...)` is last-declaration-wins; `match` / `asVariant` see only the tag. IDE `malda-types` Warning (`--strict-types` Error). | `r.Ok("Ciao")` / `r2.Ok("Ciao")`, or unique constructor names. After `import { r }`, use `r.Ok` — constructors are not flattened |
...
| `decomposeTask("…")` then `executePlan(plan, team)` | The default decomposer prompt has no `role` field. Team execution then fails with `missing role`. | Pass the team: `decomposeTask(instruction, client, team)` or `team.decompose(instruction, client?)`. |
| `team.handoff(from, to, payload, true)` expecting to stay offline | `think: true` calls `to.think(payload)`. Agents from `define`/`team` still have a conversation object, so this can hit the default local LLM. | Omit the fourth argument, or pass `false` / `{ think: false }`. Example: `Examples/Agents/agent_team_handoff_run.malda` |
| `team.review` / `team.reject` on a `handoff` edge | Hops are typed. `review()` needs `rel: review`; `reject()` needs `rel: reject`; `handoff()` needs `rel: handoff`. | Declare the matching edge. `{ approved: false }` / `{ rejected: true }` stay offline. Example: `Examples/Agents/agent_team_review.malda` |
| `executePlan` rewrite step after an approved review | A `reject` hop runs only when the predecessor is not approved. Default `approved` is true, so the rewrite is skipped. | Set `approved: false` on the review step (or return JSON `{ approved: false }`). Use `think: false` to stay offline. Example: `Examples/Agents/agent_team_plan_verdict.malda` |
| `executePlan` step with `out: "DraftCode"` and `think: false` but no `fixture` | The description string is not coerced. The step fails (`no fixture`) and dependents skip. | Set `fixture` to a dict or LLM-shaped JSON string. Example: `Examples/Agents/agent_team_plan_out.malda` |
| `executePlan` then reading `output` as a structured object | `output` stays a string (JSON when `out` succeeds). Loose `think()` text without `out` is unchanged. | Set `step.out` and read `results[i].data`. Use `asVariant` only if you have a tagged dict and no `out`. |
| `team.consult` on a `handoff` edge, or expecting it to move the plan | `consult` is a one-shot ask. The edge must be `rel: consult`. It does not set `approved` / `rejected` and does not transfer the plan. | Use `team.consult` on a consult edge. `delegate` / `consult` still install `addSubAgent` for tool loops. Example: `Examples/Agents/agent_team_consult.malda` |
| `agents.define({ kind: "CodingAgent" })` expecting a local GGUF download | `define` / `team` never call `DefaultLocalLlm`. The agent is constructed offline. `think()` later fails without a client. | Pass `client` as the second argument, or use `new CodingAgent(...)` when you want the default local model. Example: `Examples/Agents/agent_team_kinds.malda` |
| `new CodingAgent(...)` / `kind: "CodingAgent"` expecting `cap.*` or `policy { shell: deny }` to sandbox default tools | Constructors do not take capability tokens. Default `read_file` / `write_file` / `run_command` use a `workingDirectory` string prefix (`run_command` is created without `cap.shell`). `policy` runs on cap consume, so it does not stop those string-path tools. There is no `removeTool`. | For a prefix-only work folder, pass `workingDirectory`. For fewer tools, `DevAgent` `readOnly: true`. For unforgeable limits, `new Agent` + `createRunCommandTool(dir, cap.shell(...))` / `createWebFetchTool(httpCap)` / `@Tool` + `cap.confine`. Manual: `ReferenceManual/18-agent-orchestration.html` specialized kinds. Scaffold: `malda new agent` |
| `kind: "Wizard"` or `workingDirectory` on a plain `Agent` | Unknown kinds throw. `workingDirectory` is rejected on `kind: "Agent"`. `includeSymbols` / `readOnly` / `prdAuthorOnly` are `DevAgent` only. | Use `Agent`, `CodingAgent`, `GitAgent`, `HumanAgent`, `DevAgent`, or `MALDACodingAgent`. |
| `orchestrator.addSubAgent(...)` as the only org chart | The supervisor can call any installed subagent. Relations live in prompt text. | Prefer `agents.team(specs, graph directed { … })` so hops are an allow-list. `delegate` / `consult` edges still install `addSubAgent` |
...
| `suite` / `case` / `expect` under `malda run` | Suites are **skipped** on ordinary interpret (like `property`). The file can still print and exit 0. | `malda eval path.malda`. Few-shot: `docs/llm/few-shot/43_eval_suite.malda` |
| `expect(cond)` vs `assert` vs `validate` | `expect` throws like `assert`. `validate` returns `{ ok, error }` and does **not** throw on mismatch. | Use `expect` in eval cases; check `checked.ok` after `validate` |
| Checking only `r.error` after `validate` / `evalPrompt` | String `error` stays. The tagged variant is on **`r.agentError`**. | Read `r.agentError` when you need `SchemaMismatch` / `Timeout` / … |
...
| `malda check --fix` fixing every gotcha | `--fix` is mechanical only (flat aliases, `$"…"`, `.length()`). Other diagnostics stay. | Re-run `malda check --json` after `--fix`; edit the rest by hand |
