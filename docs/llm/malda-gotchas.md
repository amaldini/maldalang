...
| `import "typo.malda";` / `include "typo.malda";` | `include` fails at **parse** time; a file `import` fails at **run** time. Neither is silent in the IDE: `malda check --json` reports a `malda-import` Warning naming the resolved path, and F12 on the path jumps to the target. The import still runs the program with the symbol missing. | Fix the path; run `malda check path.malda --json` and look for `malda-import` |
