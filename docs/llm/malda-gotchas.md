...
| `@effects("io")` on a tool that takes a path string | The decorator is a **name allow-list**. The model can still pass `"/etc/passwd"`. | Host-mint `cap.fileRead("notes")` (or a workspace root) and `cap.confine` a **relative** path from tool args. `cap.read` rejects strings and forged `{ kind, path }` dicts. Do not put a token in LLM JSON. Scaffold: `malda new agent`. Example: `Examples/Tools/capability_tokens.malda` |
...
| `evalPrompt(p, fixture)` expecting an LLM call | No model, no `@budget` consumption, no gather round. Bad fixtures return `{ ok: false, error }` instead of throwing (unlike `await` after repair). | That is the harness. Use `await` / `runPrompt` when you want the model |
