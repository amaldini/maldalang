...
`three_shader_billiards.malda` is a playable GPU ray-traced pool table on the same `three.createShaderMaterial` path as `three_shader_raytracer.malda`. Host MALDA aims the cue and steps 2D circle physics (cushions, pockets, friction); the kernel traces `uBall0`–`uBall15` plus the cue stick. Digit decals stay procedural. The cue stays locked on the white ball; mouse on the table aims (A/D fine-tunes). Hold click or Space to charge power, release to shoot; R reracks; arrows orbit.
