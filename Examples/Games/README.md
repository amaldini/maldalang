...
`three_shader_billiards.malda` is a GPU ray-traced pool table (felt, cushions, pockets, numbered 16-ball rack) on the same `three.createShaderMaterial` path as `three_shader_raytracer.malda`. Rest poses and digit decals live in the kernel; it is a render showcase, not pool physics.
