...
`three_shader_mandelbrot.malda` is an infinite Seahorse Valley zoom on the same `three.createShaderMaterial` / `@shader()` path. The host builds a BigInt reference orbit (`three.mandelbrotOrbit`, decimal strings for C) and the kernel applies scaled perturbation so the dive stays sharp past float32 ulp, wrapping after 10 decades. Hold Space to pause; `[` `]` change speed.
